  ...
  for (let i = 0; i < 8; i++) {
    ...
    for (let j = 0; j < imageDescriptions.length; j++) {
      ...
      img.style.backgroundImage =
        "linear-gradient(45deg, hsla(0, 0%, 0%, 0.2) 25%, transparent 25%, transparent 50%, hsla(0, 0%, 0%, 0.2) 50%, hsla(0, 0%, 0%, 0.2) 75%, transparent 75%, transparent)";
      img.style.backgroundSize = "20px 20px";
      img.innerHTML = `<span style="color: white; font-weight: bold; text-shadow: 0 0 5px black;">Image ${
        j + 1
      }</span>`;
...
function zoomToPlanet(planet) {
  ...
  const cameraOffset = planetPos
    .clone()
    .normalize()
    .multiplyScalar(planet.userData.originalSize * 3);
